...
export default function Demo() {
    ...
    useEffect(() => {
        const fetchChains = async () => {
            try {
                console.log('🔗 Fetching chains...', {selectedChainId, urlChainId});
                const response = await fetch('/api/chains');
                const data = await response.json();
                if (data.success && data.chains) {
                    console.log('📋 Available chains:', data.chains.map(c => ({
                        id: c.id,
                        name: c.networkName,
                        isActive: c.isActive
                    })));
                    ...
                    // Only set chain if we have a URL chain ID and don't already have one selected
                    if (!selectedChainId && urlChainId) {
                        const targetChain = data.chains.find((chain: any) => chain.id.toString() === urlChainId);
                        if (targetChain) {
                            console.log('🎯 Setting chain from dashboard:', targetChain.networkName, 'ID:', urlChainId);
                            setSelectedChainId(urlChainId);
                            setNetworkName(targetChain.networkName);
                            // Clear ZK Account info when setting chain from URL
                            setZkAccountInfo(null);
                        } else {
                            console.log('⚠️ Target chain not found for URL chainId:', urlChainId);
                        }
                    } else if (!selectedChainId && !urlChainId) {
                        // Only set active chain if no URL chain and no selected chain
                        const activeChain = data.chains.find((chain: any) => chain.isActive);
                        if (activeChain) {
                            console.log('🌟 Setting default active chain:', activeChain.networkName, 'ID:', activeChain.id);
                            setSelectedChainId(activeChain.id.toString());
                            setNetworkName(activeChain.networkName);
                            setZkAccountInfo(null);
                        }
                    } else {
                        console.log('🔒 Chain selection blocked - selectedChainId:', selectedChainId, 'urlChainId:', urlChainId);
                    }
                }
            } catch (error) {
    ...
    useEffect(() => {
        ...
        // PREVENT MULTIPLE CALLS: Only run if we have a wallet, chain, and chains data
        if (wallet && selectedChainId && chains.length > 0) {
            // DEBOUNCE: Create a unique identifier to prevent race conditions
            const effectId = `${wallet.address}-${selectedChainId}-${Date.now()}`;
            console.log('🎯 Starting balance fetch for effect:', effectId);
            ...
            const fetchBalance = async () => {
                ...
                try {
                    ...
                    if (selectedChain) {
                        console.log('💰 [' + effectId + '] Fetching balance for chain:', selectedChain.networkName, 'ID:', selectedChainId);
                        const provider = new ethers.JsonRpcProvider(selectedChain.rpcUrl);
                        const balance = await getWalletBalance(wallet.address, provider);
                        setWalletBalance(balance.formatted);
                        setBalanceError("");

                        // Check if ZK Account exists on this chain ONLY if we're on the balance step
                        if (userEmail && currentStep === "balance") {
                            console.log('🔍 [' + effectId + '] Checking for ZK Account on chain:', selectedChain.networkName, 'with chainId:', selectedChainId);
                            // Clear previous ZK Account info before checking new chain
                            setZkAccountInfo(null);

                            try {
                                const zkInfo = await checkZKAccount(wallet.address, selectedChainId);
                                console.log('📊 [' + effectId + '] ZK Account result:', zkInfo.hasZKAccount ? 'FOUND' : 'NOT FOUND');

                                if (zkInfo.hasZKAccount) {
                                    setZkAccountInfo(zkInfo);
                                    console.log('✅ [' + effectId + '] Found ZK Account on this chain:', zkInfo.zkAccountAddress);
                                    ...
                                    console.log('❌ [' + effectId + '] No ZK Account on this chain');
                                }
                            } catch (error) {
                        ...
                        console.log('⚠️ [' + effectId + '] Chain not found for selectedChainId:', selectedChainId);
                    }
                } catch (error) {
            ...
            console.log('🚫 Balance effect skipped - missing requirements:', {
                hasWallet: !!wallet,
                hasSelectedChainId: !!selectedChainId,
                hasChainsData: chains.length > 0
            });
        }
    }, [wallet, selectedChainId, userEmail, currentStep]);
            ...
            if (userEmail && selectedChainId) {
                ...
                try {
                    ...
                    if (existingAccount.hasZKAccount) {
                        ...
                        // Use window.location instead of setLocation to ensure proper redirect
    ...
    const handleRequestGasFee = async () => {
        if (!wallet || !userEmail || !selectedChainId) {
            alert('Missing wallet, user email, or selected chain');
            return;
        }
                            ...
                            <div
                                ...
                                <div className="bg-card rounded-lg p-8 max-w-sm w-full mx-4 shadow-lg">
                                    <div className="flex flex-col items-center space-y-4">
                                        ...
                                        <h3 className="text-lg font-semibold text-foreground">Setting up your
                                            wallet</h3>
                        ...
                        <div className="w-full space-y-6">
                            ...
                            <div className="bg-card rounded-lg p-6 space-y-4">
                                ...
                                <div className="space-y-3">
                                    <div>
                                        <Label className="text-sm font-medium text-muted-foreground">Public
                                            Address</Label>
                                    ...
                                    <div>
                                        ...
                                        {balanceError ? (
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-muted-foreground text-xs">Click refresh to try
                                                    again</p>
                                            </div>
                                        ) : (
                                ...
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 border-b border-border">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-xs text-muted-foreground">Smart contract address for ZK
                                                operations</p>
                                        </div>
                      ...
                      <span className="text-sm text-foreground font-mono break-all flex-1 mr-2">
                        {zkAccountInfo?.zkAccountAddress || zkCreationResult?.zkAccountAddress || '0x1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b'}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 border-b border-border">
                                        <div className="space-y-1">
                                            <span
                                                className="text-sm font-medium text-muted-foreground">Owner (EOA)</span>
                                            <p className="text-xs text-muted-foreground">Externally Owned Account
                                                address</p>
                                        </div>
                                        <div className="md:col-span-2 flex items-center justify-between">
                      <span className="text-sm text-foreground font-mono break-all flex-1 mr-2">
                        {wallet?.address || '0x9b8a7c6d5e4f3a2b1c0d9e8f7a6b5c4d3e2f1a0b'}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 border-b border-border">
                                        <div className="space-y-1">
                                            <span
                                                className="text-sm font-medium text-muted-foreground">ETH Balance</span>
                                            <p className="text-xs text-muted-foreground">Current Ethereum balance</p>
                                        </div>
                                        <span
                                            className="text-sm text-foreground md:col-span-2">{zkAccountInfo?.balance || '0'} ETH</span>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 border-b border-border">
                                        <div className="space-y-1">
                                            <span className="text-sm font-medium text-muted-foreground">OA3 Token Balance</span>
                                            <p className="text-xs text-muted-foreground">OAuth3 token balance in ZK
                                                Account</p>
                                        </div>
                                        <span
                                            className="text-sm text-foreground md:col-span-2">{zkAccountInfo?.tokenBalance || '0'} OA3</span>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 border-b border-border">
                                        <div className="space-y-1">
                                            <span className="text-sm font-medium text-muted-foreground">Requires ZK Proof</span>
                                            <p className="text-xs text-muted-foreground">Zero-Knowledge verification
                                                required</p>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 border-b border-border">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-xs text-muted-foreground">Cryptographic hash
                                                of {userEmail}</p>
                                        </div>
                                        <div className="md:col-span-2 flex items-center justify-between">
                      <span className="text-sm text-foreground font-mono break-all flex-1 mr-2">
                        {zkAccountInfo?.emailHash || zkCreationResult?.emailHash || '0xabcdef1234567890abcdef1234567890abcdef1234567890'}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-xs text-muted-foreground">Cryptographic hash
                                                of {userEmail ? userEmail.split('@')[1] : 'gmail.com'}</p>
                                        </div>
                                        <div className="md:col-span-2 flex items-center justify-between">
                      <span className="text-sm text-foreground font-mono break-all flex-1 mr-2">
